...
<link rel="stylesheet"
	href="<%= contextPath %>/resources/static/css/common.css">
<script
...
<body>
	...
	<nav class="navbar navbar-expand-md">
		<a class="navbar-brand" href="#"><img
			src="<%= contextPath%>/resources/static/<%=userDto.getUserProfileImageUrl() %>"
			alt="Logo" style="width: 40px; border-radius: 50%;"></a>
...
// 		var regDtStr = regDt;
// 		var regDtStr = makeDateStr(regDt.year, regDt.monthValue, regDt.dayOfMonth, '.'); // for Gson Format Of LocalDateTime
...
function boardDetail(boardId, userSeq){
	...
	{
        type : 'get',
        url : '<%= contextPath%>/boards/' + boardId,
        dataType : 'json',
        data : {
		},
        success : function(data, status, xhr) { 
	...
	if( fileList.length > 0 ){
		for(var i=0; i<fileList.length; i++){
			...
			$("#fileListDetail").append(
					...
					'data-fileId="' + fileId + '" ' +
					'href="<%= contextPath%>/resources/static/' + fileUrl + '" ' +
...
function boardUpdate(){
	...
	{
		type : 'POST',
        url : '<%=contextPath%>/boards/' + $("#boardUpdateModal").attr("data-boardId"),
        dataType : 'json',
        data : formData,
        contentType: false,
        processData: false,
...
function boardDelete(){
	...
	{
        type : 'POST',
        url : '<%=contextPath%>/boards/' + $("#boardDetailModal").attr("data-boardId"),
        dataType : 'json',
        data : 
		{
        	_method : 'DELETE',
