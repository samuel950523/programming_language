<template>
    <div id="app">
        ...
        <button v-on:click="noticeCreate">공지사항 쓰기</button>
        ...
        <table class="table table-hover mt-2">
            ...
            <tbody>
                <tr v-for="(list, index) in noticeList" :key="index">
                    ...
                    <td><button v-on:click="noticeEdit(index)">수정</button></td>
                    <td><button v-on:click="noticeDelete(index)">삭제</button></td>
                </tr>
            </tbody>
        </table>
        <!-- <div class="btn-cover">
    ...
    methods:{
        makelistprops: function(notices){
            ...
            for(var i in notices.data) {
                result.push({ id:notices.data[i].id, title:notices.data[i].title , content:notices.data[i].content, createtime:notices.data[i].createtime})
            }   
            return result;
        },
        ...
        noticeEdit: function (index) {
            ...
            this.$router.push({name: 'BoardEdit_', params: {id: id, title: title, content: content}})
        },
